...
def translate_full_text_natural(full_text, target_lang=DEFAULT_TRANSLATE_LANG):
    """برخلاف translate_segments (که برای هم‌ترازی زیرنویس تیکه‌تیکه می‌کنه)، این یکی
    کل متن رو یکجا و روون ترجمه می‌کنه — مخصوص صدا (TTS)، با لحن طبیعی و علامت‌گذاری درست."""
    prompt = (
        f"متن زیر رو به‌صورت یه روایت کامل، روون و طبیعی (نه تکه‌تکه) به زبان با کد {target_lang} "
        f"ترجمه کن. از علامت‌های نگارشی (نقطه، ویرگول) درست استفاده کن تا موقع خونده‌شدن با صدای "
        f"مصنوعی، طبیعی و بدون مکث‌های عجیب به‌نظر بیاد. فقط متن ترجمه‌شده رو بده، بدون توضیح اضافه:\n\n"
        f"{full_text}"
    )
    try:
...
def translate_segments(segments, target_lang=DEFAULT_TRANSLATE_LANG):
    ...
    prompt = (
        f"متن زیر با جداکننده‌ی ' ||| ' به {len(segments)} بخش تقسیم شده. هر بخش رو با لحن "
        f"محاوره‌ای و طبیعی (مثل فارسی روزمره‌ی تهرانی، نه رسمی و کتابی) و روان به زبان با کد "
        f"{target_lang} ترجمه کن — از کلمات رباتیک و ترجمه‌ی کلمه‌به‌کلمه پرهیز کن. دقیقاً با همون "
        f"جداکننده‌ی ' ||| ' بین بخش‌های ترجمه‌شده جدا کن. تعداد بخش‌های خروجی باید دقیقاً "
        f"{len(segments)} تا باشه، بدون هیچ توضیح اضافه:\n\n{joined}"
    )
    try:
...
def handle_media_message(chat_id, media):
    ...
    translated_parts = translate_segments(segments)
    if translated_parts:
        translated_segments = [
            {"start": s["start"], "end": s["end"], "text": t}
            for s, t in zip(segments, translated_parts)
        ]
        send_document(chat_id, "translated.srt", build_srt(translated_segments))
        send_document(chat_id, "translated.vtt", build_vtt(translated_segments))
        send_document(chat_id, "translated.sbv", build_sbv(translated_segments))
        translated_full_text = build_txt(translated_segments)
        send_document(chat_id, "translated.txt", translated_full_text)

        natural_text = translate_full_text_natural(build_txt(segments))
        tts_source_text = natural_text or translated_full_text
        try:
            with tempfile.TemporaryDirectory() as tts_tmpdir:
                ...
                send_audio_file(chat_id, audio_out_path, caption="صدای فارسیِ ترجمه")
        except Exception as e:
            print(f"خطا در تولید صدای فارسی: {e}")
            send_message(chat_id, "متن ترجمه آماده شد ولی تولید صدای فارسی موفق نشد.")
    else:
        send_message(chat_id, "⚠️ هم‌ترازی زمان‌بندی ترجمه موفق نشد؛ فقط متن اصلی (بدون ترجمه) رو داری.")
...
def handle_update(update):
    ...
    if not media and doc:
        ...
        if mime.startswith(("video/", "audio/")):
            ...
            handle_image_optimize(chat_id, doc["file_id"], (message.get("caption") or "").strip())
            return
    if media:
