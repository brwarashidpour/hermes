...
def check_cloudflare_ip(ip, timeout=4):
    """یه درخواست HTTP ساده (نه صدها اتصال موازی) می‌زنه؛ اگه واقعاً کلادفلر
    باشه (هدر cf-ray داشته باشه) و جواب بده، تأخیرش رو به میلی‌ثانیه برمی‌گردونه."""
    try:
        start = time.time()
        resp = requests.get(
        ...
        if resp.status_code == 200 and "cf-ray" in {k.lower() for k in resp.headers}:
            return elapsed
    except Exception:
        pass
    return None
