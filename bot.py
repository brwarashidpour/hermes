...
def send_audio_file(chat_id, file_path, caption=""):
    try:
        ...
        if _current_business_connection_id:
            data["business_connection_id"] = _current_business_connection_id
        ...
        if not resp.ok:
            print(f"خطا در ارسال صدا: {resp.status_code} {resp.text[:300]}")
            send_message(chat_id, f"⚠️ فایل صوتی ارسال نشد (خطای تلگرام: {resp.status_code}).")
    except Exception as e:
        print(f"خطا در ارسال صدا: {e}")
        send_message(chat_id, f"⚠️ فایل صوتی ارسال نشد: {e}")
...
def send_document(chat_id, filename, content_str):
    try:
        ...
        if not resp.ok:
            print(f"خطا در ارسال فایل {filename}: {resp.status_code} {resp.text[:300]}")
            send_message(chat_id, f"⚠️ فایل «{filename}» ارسال نشد (خطای تلگرام: {resp.status_code}).")
    except Exception as e:
        print(f"خطا در ارسال فایل {filename}: {e}")
        send_message(chat_id, f"⚠️ فایل «{filename}» ارسال نشد: {e}")
...
# وقتی یه پیام از طریق Telegram Business میاد، جواب باید حتماً همراه
# business_connection_id باشه؛ چون توابع send_* همه‌جا فقط با chat_id صدا زده
# می‌شن (نه با پاس دادن این مقدار به هرکدوم)، یه متغیر سطح‌ماژول نگهش می‌داریم —
# چون فقط یه پیام در لحظه پردازش می‌شه (حلقه‌ی تک‌رشته‌ای)، امن‌ه.
_current_business_connection_id = None
...
def send_message(chat_id, text, reply_markup=None):
    for i in range(0, len(text), TELEGRAM_MAX_LEN):
        ...
        if _current_business_connection_id:
            payload["business_connection_id"] = _current_business_connection_id
        try:
...
TIKTOK_LINK_RE = re.compile(r"(?:tiktok\.com)/\S+")


def handle_tiktok_link(chat_id, link):
    try:
        resp = requests.get("https://www.tikwm.com/api/", params={"url": link}, timeout=25)
        resp.raise_for_status()
        video_url = (resp.json().get("data") or {}).get("play")
        if not video_url:
            send_message(chat_id, "ویدیویی تو این لینک پیدا نشد.")
            return
    except Exception as e:
        send_message(chat_id, f"خطا در پیدا کردن ویدیو: {e}")
        return
    try:
        requests.post(f"{API_URL}/sendVideo",
                       json={"chat_id": chat_id, "video": video_url}, timeout=60)
    except Exception as e:
        send_message(chat_id, f"خطا در ارسال ویدیو: {e}")
...
def handle_xvideo_link(chat_id, link):
    ...
    try:
        resp = requests.get(f"https://api.fxtwitter.com/status/{tweet_id}", timeout=20)
        resp.raise_for_status()
...
# نمونه‌ای از IPهای منتشرشده‌ی رسمی کلادفلر (cloudflare.com/ips-v4)، چند تا از
# چند رنج مختلف — نه اسکن تصادفی هزاران‌تایی، فقط یه چک سبک و سریع.
CLOUDFLARE_SAMPLE_IPS = [
    "104.16.0.1", "104.17.0.1", "104.18.0.1", "104.19.0.1", "104.20.0.1",
    "104.21.0.1", "104.24.0.1", "104.26.0.1", "104.27.0.1", "172.64.0.1",
    "172.65.0.1", "172.66.0.1", "172.67.0.1", "162.159.0.1", "198.41.128.1",
]


def check_cloudflare_ip(ip, port=443, timeout=4, sni_host="speed.cloudflare.com"):
    """یه اتصال HTTPS خام می‌زنه با کنترل مستقیم SNI (چیزی که requests library
    اجازه نمی‌ده — Host header فقط لایه‌ی HTTP رو عوض می‌کنه، نه SNI تو لایه‌ی TLS،
    و کلادفلر بدون SNI درست، اتصال رو رد می‌کنه). اگه واقعاً کلادفلر باشه و جواب
    بده، تأخیرش رو به میلی‌ثانیه برمی‌گردونه."""
    import socket
    ...
    try:
        ...
        if data.startswith(b"HTTP/1.1 200") and b"cloudflare" in data.lower():
            return elapsed
    except Exception:
        pass
    return None
...
VLESS_LINE_RE = re.compile(r"vless://[^\s]+")


def parse_vless(link):
    """از یه لینک vless://، آی‌پی، پورت، و SNI رو درمیاره تا بشه تستش کرد."""
    m = re.match(r"vless://[^@]+@([^:/?]+):(\d+)", link)
    if not m:
        return None
    ip, port = m.group(1), int(m.group(2))
    sni_m = re.search(r"[?&]sni=([^&#]+)", link)
    sni = sni_m.group(1) if sni_m else ip
    return ip, port, sni


def test_my_proxy(link, timeout=4):
    parsed = parse_vless(link)
    if not parsed:
        return None
    ip, port, sni = parsed
    latency = check_cloudflare_ip(ip, port=port, timeout=timeout, sni_host=sni)
    return latency


def parse_vless_full(link):
    """کل جزئیات یه لینک vless:// رو درمیاره — برای ساخت JSON کامل هر سرور."""
    m = re.match(r"vless://([^@]+)@([^:/?]+):(\d+)", link)
    if not m:
        return None
    uuid, ip, port = m.group(1), m.group(2), int(m.group(3))

    def q(param, default=""):
        found = re.search(rf"[?&]{param}=([^&#]+)", link)
        return found.group(1) if found else default

    frag_m = re.search(r"#(.+)$", link)
    remark = frag_m.group(1) if frag_m else ip

    return {
        "uuid": uuid, "ip": ip, "port": port,
        "security": q("security", "none"), "sni": q("sni", ip),
        "fp": q("fp", "chrome"), "net_type": q("type", "tcp"),
        "host": q("host", ""), "path": q("path", "/"),
        "remark": remark,
    }


def build_vless_outbound(parsed):
    """یه outbound استاندارد Xray برای یه سرور می‌سازه (بخش هسته‌ی هر پروفایل)."""
    outbound = {
        "tag": parsed["remark"],
        "protocol": "vless",
        "settings": {
            "vnext": [{
                "address": parsed["ip"],
                "port": parsed["port"],
                "users": [{"id": parsed["uuid"], "encryption": "none", "flow": ""}],
            }]
        },
        "streamSettings": {"network": parsed["net_type"], "security": parsed["security"]},
    }
    if parsed["security"] == "tls":
        outbound["streamSettings"]["tlsSettings"] = {
            "serverName": parsed["sni"], "fingerprint": parsed["fp"], "allowInsecure": False,
        }
    if parsed["net_type"] == "ws":
        outbound["streamSettings"]["wsSettings"] = {
            "path": parsed["path"], "headers": {"Host": parsed["host"] or parsed["sni"]},
        }
    return outbound


def build_combined_json(links):
    """همه‌ی سرورها رو به‌عنوان آرایه‌ای از outboundهای Xray، تو یه فایل JSON واحد
    برمی‌گردونه — یه outbound به‌عنوان پیش‌فرض فعال (اولین سرور)، بقیه به‌عنوان
    گزینه‌های قابل‌سوییچ تو همون فایل."""
    outbounds = []
    for link in links:
        parsed = parse_vless_full(link)
        if parsed:
            outbounds.append(build_vless_outbound(parsed))
    outbounds.append({"tag": "direct", "protocol": "freedom"})
    return {
        "remarks": f"{len(outbounds) - 1} سرور",
...
def handle_new_proxy_configs(chat_id, found_links):
    """کانفیگ‌های vless:// جدید رو ذخیره می‌کنه و **کل** لیست ذخیره‌شده (نه فقط
    جدیدها) رو تو یه فایل JSON واحد (شامل همه‌ی سرورها) برمی‌گردونه."""
    data = load_all()
    my_proxies = data.get("_my_proxies", {})
    existing = my_proxies.get(chat_id, [])
    new_unique = [link for link in found_links if link not in existing]
    all_links = existing + new_unique
    my_proxies[chat_id] = all_links
    data["_my_proxies"] = my_proxies
    save_all(data)

    combined = build_combined_json(all_links)
    send_document(chat_id, "servers.json", json.dumps(combined, indent=2, ensure_ascii=False))

    send_message(chat_id,
        f"{len(new_unique)} کانفیگ جدید اضافه شد (مجموع: {len(all_links)}). یه فایل JSON با "
        f"**همه‌ی** سرورها (به‌صورت چندتا outbound تو یه فایل) فرستادم.\n"
        f"تو NapsternetV: Import → From file → همین فایل.\n"
        f"⚠️ اگه ایمپورت چندسرورهه رو این فرمت دقیق جواب نداد (چون این ساختار خودساخته‌ست، نه "
        f"مستندات رسمی NapsternetV)، مطمئن‌ترین جایگزین اینه که بگی تا فایل ساده‌ی متنی "
        f"(هر خط یه vless://، که همه‌ی این اپ‌ها بدون شک می‌شناسنش) رو هم بفرستم.\n"
        f"برای تست سرعت همه‌ی سرورها: /myproxies")
...
def find_persian_font():
    """مسیر واقعی فونت نصب‌شده رو با fontconfig پیدا می‌کنه — به‌جای هاردکد کردن
    یه مسیر ثابت که ممکنه بین نسخه‌های اوبونتو فرق کنه."""
    global _PERSIAN_FONT_PATH
    if _PERSIAN_FONT_PATH:
        return _PERSIAN_FONT_PATH
    try:
        result = subprocess.run(
            ["fc-match", "-f", "%{file}", "Noto Sans Arabic"],
            capture_output=True, text=True, timeout=10,
        )
        path = result.stdout.strip()
        if path and os.path.exists(path):
            _PERSIAN_FONT_PATH = path
    except Exception as e:
        print(f"خطا در پیدا کردن فونت فارسی: {e}")
    return _PERSIAN_FONT_PATH


def shape_farsi(text):
    """حروف فارسی رو به شکل متصل‌شده‌ی درست تبدیل می‌کنه و ترتیب راست‌به‌چپ رو
    اصلاح می‌کنه — بدون این کار، کتابخونه‌ی PDF حروف رو جدا و برعکس می‌نویسه."""
    reshaped = arabic_reshaper.reshape(text)
    return get_display(reshaped)


def build_checks_pdf(checks, output_path):
    ...
    if font_path:
        ...
        pdf.set_font("Persian", size=14)
    else:
        pdf.set_font("Helvetica", size=12)  # فقط اگه فونت فارسی پیدا نشد، جایگزین موقت

    page_width = pdf.w - 2 * pdf.l_margin

    title = shape_farsi("لیست چک‌ها") if font_path else "Checks List"
    pdf.cell(page_width, 12, title, align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)

    for c in checks:
        header = f"#{c['id']} — {c.get('due_jalali', c.get('due', ''))}"
        line = shape_farsi(header) if font_path else header
        pdf.set_font(pdf.font_family, size=13)
        pdf.multi_cell(page_width, 9, line, align="R" if font_path else "L")

        desc = c.get("desc") or ""
        if desc:
            pdf.set_font(pdf.font_family, size=11)
            for raw_line in desc.split("\n"):
                shaped = shape_farsi(raw_line) if font_path else raw_line
                pdf.multi_cell(page_width, 7, shaped, align="R" if font_path else "L")

        if c.get("photo_file_id"):
            note = shape_farsi("(عکس چک هم ضمیمه‌ست)") if font_path else "(has photo)"
            pdf.set_font(pdf.font_family, size=9)
            pdf.multi_cell(page_width, 6, note, align="R" if font_path else "L")
...
def send_document_from_path(chat_id, file_path, filename):
    try:
        data = {"chat_id": chat_id}
        if _current_business_connection_id:
            data["business_connection_id"] = _current_business_connection_id
        with open(file_path, "rb") as f:
            resp = requests.post(
                f"{API_URL}/sendDocument", data=data,
                files={"document": (filename, f)}, timeout=60,
            )
        if not resp.ok:
            print(f"خطا در ارسال فایل {filename}: {resp.status_code} {resp.text[:300]}")
            send_message(chat_id, f"⚠️ فایل «{filename}» ارسال نشد (خطای تلگرام: {resp.status_code}).")
...
def handle_callback_query(cq):
    ...
    if data_str.startswith("nav:"):
        _, target, page_str = data_str.split(":", 2)
        page = int(page_str)
        if target == "main":
...
def handle_update(update):
    ...
    if user_text == "/checkspdf":
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            pdf_path = os.path.join(tmpdir, "checks.pdf")
            try:
                had_font = build_checks_pdf(my_checks, pdf_path)
            except Exception as e:
                send_message(chat_id, f"خطا در ساخت PDF: {e}")
                return
            send_document_from_path(chat_id, pdf_path, "checks.pdf")
            if not had_font:
                send_message(chat_id, "⚠️ فونت فارسی رو سرور پیدا نشد، متن انگلیسی/ساده جایگزین شد.")
        return
    ...
    if TIKTOK_LINK_RE.search(user_text):
        handle_tiktok_link(chat_id, user_text)
        return

    if user_text.startswith("/addproxy"):
        found = VLESS_LINE_RE.findall(user_text)
        if not found:
            send_message(chat_id, "هیچ لینک vless:// معتبری پیدا نشد. یه یا چند لینک vless:// بفرست.")
            return
        handle_new_proxy_configs(chat_id, found)
        return

    if VLESS_LINE_RE.search(user_text) and not user_text.startswith("/"):
        found = VLESS_LINE_RE.findall(user_text)
        handle_new_proxy_configs(chat_id, found)
        return

    if user_text == "/myproxies":
        data = load_all()
        links = data.get("_my_proxies", {}).get(chat_id, [])
        if not links:
            send_message(chat_id, "هنوز کانفیگی ذخیره نکردی. یه لینک vless:// بفرست یا /addproxy بزن.")
            return
        send_message(chat_id, f"در حال تست {len(links)} کانفیگ... چند ثانیه طول می‌کشه.")
        results = []
        for link in links:
            latency = test_my_proxy(link)
            if latency is not None:
                results.append((link, latency))
        if not results:
            send_message(chat_id, "هیچ‌کدوم جواب ندادن (از دید سرور گیت‌هاب) — ممکنه موقتی باشه.")
            return
        results.sort(key=lambda x: x[1])
        sorted_text = "\n".join(link for link, _ in results)
        send_document(chat_id, "my_proxies_sorted.txt", sorted_text)
        summary = "\n".join(f"{i+1}. {latency:.0f}ms" for i, (_, latency) in enumerate(results[:10]))
        send_message(chat_id, f"سریع‌ترین‌ها (فایل کامل هم فرستادم، بهترین بالای فایله):\n\n{summary}")
        return

    if user_text.startswith("/removeproxies"):
        data = load_all()
        my_proxies = data.get("_my_proxies", {})
        my_proxies[chat_id] = []
        data["_my_proxies"] = my_proxies
        save_all(data)
        send_message(chat_id, "همه‌ی کانفیگ‌های ذخیره‌شده‌ت پاک شد.")
        return

    if user_text == "/cleanip":
        send_message(chat_id, "در حال چک کردن IPهای کلادفلر (از سرور خارجی)... چند ثانیه طول می‌کشه.")
        results = []
        for ip in CLOUDFLARE_SAMPLE_IPS:
            latency = check_cloudflare_ip(ip)
            if latency is not None:
                results.append((ip, latency))
        if not results:
            send_message(chat_id, "هیچ‌کدوم جواب ندادن — احتمالاً یه مشکل شبکه‌ای موقتیه.")
            return
        results.sort(key=lambda x: x[1])
        top = results[:10]
        lines = [f"{ip} — {latency:.0f}ms" for ip, latency in top]
        send_message(chat_id,
            "سریع‌ترین IPها (از دید سرور خارجی — تست نهایی رو حتماً خودت با v2rayNG/Hiddify "
            "رو شبکه‌ی خودت انجام بده):\n\n" + "\n".join(lines))
        return

    if user_text == "/proxyconfigs":
        send_message(chat_id, "در حال گرفتن لیست کانفیگ‌ها...")
        try:
            resp = requests.get(
            ...
            send_message(chat_id, f"خطا در گرفتن لیست: {e}")
            return
        send_document(chat_id, "configs.txt", content)
        send_message(chat_id,
            "این فایل خام و تست‌نشده‌ست (بدون فیلتر پینگ/سرعت). محتواشو تو v2rayNG یا "
            "Hiddify از حالت «Import from clipboard» وارد کن — خودِ اپ کانفیگ‌های سریع رو مرتب می‌کنه.")
        return
    ...
    if user_text.startswith("/price"):
        ...
        try:
            ...
            send_message(chat_id, f"خطا در گرفتن قیمت: {e}")
            return
        if not prices:
    ...
    if user_text == "/help":
        send_message(chat_id,
            ...
            "/proxyconfigs - گرفتن لیست کانفیگ V2Ray/Xray عمومی (خام، تست‌نشده)\n"
            "/cleanip - چک سریع چندتا IP کلادفلر (فقط تست اولیه، تست نهایی رو خودت رو شبکه‌ات انجام بده)\n\n"
            "🔗 کانفیگ‌های vless:// خودتو (تک یا چندتایی) مستقیم بفرست تا ذخیره بشن و یه فایل با همه‌ی سرورها بگیری\n"
            "🔗 /myproxies - تست همه‌ی کانفیگ‌های ذخیره‌شده‌ت و گرفتن سریع‌ترین‌ها\n"
            "🔗 /removeproxies - پاک کردن همه‌ی کانفیگ‌های ذخیره‌شده\n\n"
            ...
            "💰 /checkspdf - همون لیست، تو یه فایل PDF\n"
...
def send_photo(chat_id, file_id, caption=""):
    try:
        ...
        if _current_business_connection_id:
            payload["business_connection_id"] = _current_business_connection_id
        resp = requests.post(f"{API_URL}/sendPhoto", json=payload, timeout=15)
        if not resp.ok:
            print(f"خطا در ارسال عکس: {resp.status_code} {resp.text[:300]}")
            send_message(chat_id, f"⚠️ عکس ارسال نشد (خطای تلگرام: {resp.status_code}).")
    except Exception as e:
        print(f"خطا در ارسال عکس: {e}")
        send_message(chat_id, f"⚠️ عکس ارسال نشد: {e}")
...
def check_reminders():
    ...
    چک‌هایی که تاریخشون گذشته، خودکار از حافظه حذف می‌شن — ولی اگه هیچ‌وقت یادآوری‌شون
    فرستاده نشده بود (مثلاً تاریخ همون روز یا گذشته وارد شده، یا بات یه روز کامل قطع بوده)،
    قبل از حذف یه پیام «یادآوری عقب‌افتاده» می‌فرستیم تا بی‌خبر گم نشه."""
    data = load_all()
    checks = data.get("_checks", [])
    if not checks:
        return
    ...
    for c in checks:
        ...
        if days_left < 0:
            if not c.get("reminded_date"):
                text = (f"⚠️ چک «{c.get('desc') or 'بدون توضیح'}» سررسیدش ({c.get('due_jalali', c['due'])}) "
                        f"گذشته و قبلش یادآوری نفرستاده بودم (شاید تاریخش همون روز/گذشته ثبت شده بود). "
                        f"از لیست حذفش می‌کنم.")
                recipients = [c["chat_id"]] + ([c["forward_chat_id"]] if c.get("forward_chat_id") else [])
...
def main():
    global _current_business_connection_id
    ...
    while time.time() - start_time < MAX_RUNTIME_SECONDS:
        ...
        for update in updates:
            ...
            try:
                ...
                elif update.get("business_message") or update.get("edited_business_message"):
                    biz_msg = update.get("business_message") or update.get("edited_business_message")
                    _current_business_connection_id = biz_msg.get("business_connection_id")
                    handle_update({"message": biz_msg})
                    _current_business_connection_id = None
                else:
                    _current_business_connection_id = None
