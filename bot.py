...
def handle_update(update):
    ...
    if user_text.startswith("/learn "):
        ...
        if arg.startswith("http://") or arg.startswith("https://"):
            try:
                resp = requests.get(arg, timeout=20, headers={"User-Agent": "Mozilla/5.0"})
                resp.raise_for_status()
                soup = BeautifulSoup(resp.text, "html.parser")
                content = soup.get_text("\n", strip=True)
            except Exception as e:
                send_message(chat_id, f"نتونستم این لینک رو بخونم: {e}")
                return
        else:
            content = arg
    ...
    if user_text == "/forget":
        ...
        data["_knowledge"] = ""
        save_all(data)
        send_message(chat_id, "دانش ذخیره‌شده پاک شد.")
        return
