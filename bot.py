...
def translate_segments(segments, target_lang=DEFAULT_TRANSLATE_LANG):
    ...
    prompt = (
        f"متن زیر با جداکننده‌ی ' ||| ' به {len(segments)} بخش تقسیم شده. هر بخش رو دقیق و روان "
        f"به زبان با کد {target_lang} ترجمه کن و دقیقاً با همون جداکننده‌ی ' ||| ' بین بخش‌های ترجمه‌شده "
        f"جدا کن. تعداد بخش‌های خروجی باید دقیقاً {len(segments)} تا باشه، بدون هیچ توضیح اضافه:\n\n{joined}"
    )
    try:
...
def handle_media_message(chat_id, media):
    ...
    if translated_parts:
        ...
        send_document(chat_id, "translated.txt", build_txt(translated_segments))
    else:
...
def handle_update(update):
    ...
    if not media and message.get("document"):
        doc = message["document"]
        if (doc.get("mime_type") or "").startswith(("video/", "audio/")):
            media = doc
    if media:
...
def parse_jalali_date(raw):
    """ورودی به شکل روز٫ماه٫سال با هر جداکننده‌ای (٫ ، / - فاصله) رو می‌فهمه."""
...
def parse_check_form(text):
    ...
    for line in text.split("\n"):
        ...
        if not value:
            continue
        for key_fa, key_en in CHECK_FIELD_MAP:
            if label.startswith(key_fa):
                fields[key_en] = value
                break
    return fields
...
def handle_addcheck(chat_id, command_text, photo_file_id=None):
    ...
    fields = parse_check_form(body)

    if "date" in fields:
        try:
            due_gregorian, due_jalali = parse_jalali_date(fields["date"])
        except Exception:
            send_message(chat_id, "تاریخ چک رو نتونستم بخونم. فرمت: روز٫ماه٫سال (مثلاً 24٫05٫1405)")
            return
        desc_parts = []
        if fields.get("amount_number"):
            desc_parts.append(f"مبلغ: {fields['amount_number']} ریال")
        if fields.get("amount_words"):
            desc_parts.append(f"({fields['amount_words']})")
        if fields.get("check_number"):
            desc_parts.append(f"شماره چک: {fields['check_number']}")
        if fields.get("payee"):
            desc_parts.append(f"گیرنده: {fields['payee']}")
        if fields.get("bank"):
            desc_parts.append(f"بانک: {fields['bank']}")
        desc = "\n".join(desc_parts)
    else:
    ...
    for c in checks:
        ...
        if days_left == 1 and c.get("reminded_date") != str(today):
            ...
            if c.get("photo_file_id"):
                send_photo(c["chat_id"], c["photo_file_id"], caption=text)
            else:
                send_message(c["chat_id"], text)
