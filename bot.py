...
def _make_session():
    """Session مشترک با connection pooling و retry خودکار — هم سریع‌تر (کانکشن‌ها
    دوباره استفاده می‌شن) هم مقاوم‌تر (خطاهای گذرای شبکه/429/5xx خودکار retry می‌شن)."""
    s = requests.Session()
...
def safe_err(e):
    """متن خطا رو قبل از ارسال به کاربر از توکن پاک می‌کنه — جلوی نشت BOT_TOKEN
    در پیام‌های خطایی که URL کامل دارن (مثل خطای دانلود فایل) گرفته می‌شه."""
    return str(e).replace(BOT_TOKEN, "***")
...
MAX_URL_MEDIA_BYTES = 300 * 1024 * 1024   # سقف دانلود از لینک مستقیم (/translatefile)
GROQ_MAX_AUDIO_BYTES = 24 * 1024 * 1024   # سقف عملی فایل صوتی برای Whisper
AUDIO_CHUNK_SECONDS = 600                 # تکه‌های ۱۰ دقیقه‌ای برای صداهای خیلی بلند
TRANSLATE_BATCH_SIZE = 30                 # ترجمه‌ی segmentها در دسته‌های ۳۰تایی
URL_CACHE_TTL = 600                       # کش ۱۰ دقیقه‌ای لینک‌های /learn
HEAVY_WORKERS = 4                         # تعداد چت‌هایی که همزمان پردازش می‌شن
...
def fetch_url_text(url):
    """متن یه صفحه رو می‌خونه، با کش ۱۰ دقیقه‌ای — لینک‌های /learn دیگه هر پیام
    رو چند ثانیه کند نمی‌کنن ولی محتوا همچنان تقریباً تازه می‌مونه."""
    now = time.time()
...
def send_audio_file(chat_id, file_path, caption=""):
    try:
        ...
        print(f"خطا در ارسال صدا: {safe_err(e)}")
...
def _translate_batch(batch_texts, style_instruction):
    """یه دسته از segmentها رو ترجمه می‌کنه. اول خروجی JSON می‌خواد (خیلی مقاوم‌تر
    از جداکننده‌ی متنی)، اگه پارس نشد با جداکننده‌ی ||| دوباره امتحان می‌کنه."""
    import json as _json
    payload = _json.dumps(batch_texts, ensure_ascii=False)
    prompt = (
        f"آرایه‌ی JSON زیر شامل {len(batch_texts)} رشته است. هر رشته رو با این سبک و زبان "
        f"ترجمه کن: {style_instruction}. خروجی فقط و فقط یک آرایه‌ی JSON معتبر با دقیقاً "
        f"{len(batch_texts)} رشته‌ی ترجمه‌شده (با همون ترتیب) باشه، بدون هیچ توضیح اضافه:\n\n{payload}"
    )
    try:
        reply, _, _ = get_ai_response([{"role": "user", "content": prompt}], max_tokens=4000)
        m = re.search(r"\[.*\]", reply, re.DOTALL)
        if m:
            parts = _json.loads(m.group(0))
            if isinstance(parts, list) and len(parts) == len(batch_texts):
                return [str(p).strip() for p in parts]
    except (AllProvidersFailed, ValueError):
        pass

    # مسیر دوم: جداکننده‌ی متنی
    joined = " ||| ".join(batch_texts)
    prompt2 = (
        f"متن زیر با جداکننده‌ی ' ||| ' به {len(batch_texts)} بخش تقسیم شده. هر بخش رو با این "
        f"سبک و زبان ترجمه کن: {style_instruction}. دقیقاً با همون جداکننده‌ی ' ||| ' بین "
        f"بخش‌های ترجمه‌شده جدا کن. تعداد بخش‌های خروجی باید دقیقاً {len(batch_texts)} تا باشه، "
        f"بدون هیچ توضیح اضافه:\n\n{joined}"
    )
    try:
        reply, _, _ = get_ai_response([{"role": "user", "content": prompt2}], max_tokens=4000)
    except AllProvidersFailed:
        return None
    parts = [p.strip() for p in reply.split("|||")]
    return parts if len(parts) == len(batch_texts) else None


def translate_segments_styled(segments, style):
    """ترجمه‌ی دسته‌ای: به‌جای فرستادن کل متن تو یه درخواست (که برای فایل‌های بلند
    از سقف توکن رد می‌شه و کل ترجمه از دست می‌ره)، segmentها دسته‌دسته ترجمه
    می‌شن — پس فایل‌های هر چقدر طولانی هم کامل و بدون کوتاه‌شدن ترجمه می‌شن."""
    texts = [seg["text"].strip() for seg in segments]
    out = []
    for i in range(0, len(texts), TRANSLATE_BATCH_SIZE):
        batch = texts[i:i + TRANSLATE_BATCH_SIZE]
        parts = _translate_batch(batch, style["instruction"])
        if parts is None:
            parts = _translate_batch(batch, style["instruction"])  # یه بار تلاش دوباره
        if parts is None:
            return None
        out.extend(parts)
    return out
...
NATURAL_CHUNK_CHARS = 6000


def translate_full_text_natural(full_text, instruction):
    """کل متن رو روون طبق instruction ترجمه می‌کنه — برای متن‌های بلند، تکه‌تکه
    (هر تکه ~۶۰۰۰ کاراکتر) ترجمه و به هم وصل می‌شه تا هیچ بخشی به‌خاطر
    سقف توکن حذف نشه."""
    chunks = []
    for i in range(0, len(full_text), NATURAL_CHUNK_CHARS):
        piece = full_text[i:i + NATURAL_CHUNK_CHARS]
        prompt = f"{instruction}\n\nفقط متن ترجمه‌شده رو بده، بدون توضیح اضافه:\n\n{piece}"
        try:
            translated, _, _ = get_ai_response([{"role": "user", "content": prompt}], max_tokens=4000)
        except AllProvidersFailed:
            return None
        chunks.append(translated.strip())
    return "\n".join(chunks)
...
def try_generate_speech(text, output_path, voice_candidates):
    ...
    for voice in voice_candidates:
        try:
            ...
            print(f"صدای {voice} کار نکرد: {safe_err(e)}")
            continue
    return None
...
def _media_duration_seconds(path):
    ...
    try:
        ...
        return float(AUDIO_CHUNK_SECONDS)


def _split_audio(tmpdir, audio_path, chunk_seconds=AUDIO_CHUNK_SECONDS):
    """فایل صوتی رو به تکه‌های چند دقیقه‌ای تقسیم می‌کنه (بدون encode دوباره — سریع)."""
    ...
    return sorted(os.path.join(tmpdir, f) for f in os.listdir(tmpdir) if f.startswith("chunk_"))


def transcribe_audio_long(tmpdir, audio_path):
    """اگه صدا از سقف Whisper بزرگ‌تر باشه، تکه‌تکه‌ش می‌کنه، هر تکه رو جدا transcribe
    می‌کن�� و زمان‌بندی هر تکه رو offset می‌ده — فایل‌های خیلی بلند هم کامل پردازش می‌شن."""
    if os.path.getsize(audio_path) <= GROQ_MAX_AUDIO_BYTES:
        return transcribe_audio(audio_path)

    all_segments = []
    detected_lang = "نامشخص"
    offset = 0.0
    for chunk_path in _split_audio(tmpdir, audio_path):
        segs, lang = transcribe_audio(chunk_path)
        if detected_lang == "نامشخص" and lang != "نامشخص":
            detected_lang = lang
        for s in segs:
            all_segments.append({"start": s["start"] + offset, "end": s["end"] + offset, "text": s["text"]})
        offset += _media_duration_seconds(chunk_path)
...
def send_document(chat_id, filename, content_str):
    try:
        ...
        print(f"خطا در ارسال فایل {filename}: {safe_err(e)}")


def _process_media_file(chat_id, tmpdir, input_path):
    """خط لوله‌ی مشترک ترجمه: استخراج صدا (فشرده، mono 48k) → تشخیص گفتار (با
    تکه‌تکه کردن صداهای بلند) → ترجمه‌ی موازی ۳ سبک → ارسال زیرنویس/متن/صدا."""
    audio_path = os.path.join(tmpdir, "audio.mp3")
    try:
        # صدای mono با بیت‌ریت 48k برای گفتار کاملاً کافیه و حجم رو چند برابر کم می‌کنه —
        # یعنی فایل‌های خیلی طولانی‌تری تو سقف Whisper جا می‌شن.
        subprocess.run(
            ["ffmpeg", "-y", "-i", input_path, "-vn", "-ac", "1", "-b:a", "48k",
             "-acodec", "libmp3lame", audio_path],
            check=True, timeout=600, capture_output=True,
        )
    except Exception as e:
        send_message(chat_id, f"خطا تو استخراج صدا (ffmpeg): {safe_err(e)}")
        return

    try:
        segments, detected_lang = transcribe_audio_long(tmpdir, audio_path)
    except Exception as e:
        send_message(chat_id, f"خطا تو تشخیص گفتار (Groq Whisper): {safe_err(e)}")
        return
    ...
    # ترجمه‌ی هر ۳ سبک به‌صورت موازی — زمان انتظار کاربر تقریباً یک‌سوم می‌شه
    with ThreadPoolExecutor(max_workers=len(TRANSLATION_STYLES)) as pool:
        futures = {s["key"]: pool.submit(translate_segments_styled, segments, s)
                   for s in TRANSLATION_STYLES}
        style_results = {k: f.result() for k, f in futures.items()}

    for style in TRANSLATION_STYLES:
        translated_parts = style_results.get(style["key"])
        ...
        if style["key"] == "fa_colloquial":
            ...
            try:
                ...
                log.warning(f"خطا در تولید صدای فارسی: {safe_err(e)}")
...
def handle_media_message(chat_id, media):
    ...
    if file_size and file_size > MAX_MEDIA_BYTES:
        send_message(chat_id, "این فایل از ~۱۹ مگابایت بزرگ‌تره — سقف دانلود فایل بات‌های تلگرامه.\n"
                               "راه‌حل: فایل رو یه جایی آپلود کن و لینک مستقیمش رو با این دستور بفرست:\n"
                               "/translatefile لینک (تا ۳۰۰ مگابایت پشتیبانی می‌شه)")
        return
    if not GROQ_API_KEY:
    ...
    try:
        file_info = http.get(f"{API_URL}/getFile", params={"file_id": media["file_id"]}, timeout=30).json()
        file_path = file_info["result"]["file_path"]
        file_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{file_path}"
        raw_bytes = http.get(file_url, timeout=120).content
    except Exception as e:
        send_message(chat_id, f"خطا در دانلود فایل: {safe_err(e)}")
        return
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with open(input_path, "wb") as f:
            f.write(raw_bytes)
        _process_media_file(chat_id, tmpdir, input_path)


def handle_url_media(chat_id, url):
    """دانلود فایل صوتی/ویدیویی از لینک مستقیم (تا ۳۰۰ مگابایت، استریمی) و اجرای
    همون خط لوله‌ی ترجمه — برای فایل‌هایی که از سقف ~۲۰ مگابایتی تلگرام بزرگ‌ترن."""
    if not (url.startswith("http://") or url.startswith("https://")):
        send_message(chat_id, "فرمت درست:\n/translatefile لینک-مستقیم-فایل")
        return
    if not GROQ_API_KEY:
        send_message(chat_id, "برای ترجمه به GROQ_API_KEY نیاز دارم که هنوز تنظیم نشده.")
        return
    send_message(chat_id, "در حال دانلود از لینک... (فایل بزرگ باشه چند دقیقه طول می‌کشه)")
    with tempfile.TemporaryDirectory() as tmpdir:
        input_path = os.path.join(tmpdir, "input_media")
        try:
            with http.get(url, stream=True, timeout=300, headers={"User-Agent": "Mozilla/5.0"}) as resp:
                resp.raise_for_status()
                size = 0
                with open(input_path, "wb") as f:
                    for chunk in resp.iter_content(1024 * 256):
                        size += len(chunk)
                        if size > MAX_URL_MEDIA_BYTES:
                            raise RuntimeError("فایل از سقف ۳۰۰ مگابایت بزرگ‌تره")
                        f.write(chunk)
        except Exception as e:
            send_message(chat_id, f"خطا در دانلود از لینک: {safe_err(e)}")
            return
        _process_media_file(chat_id, tmpdir, input_path)
...
def send_message(chat_id, text, reply_markup=None):
    for i in range(0, len(text), TELEGRAM_MAX_LEN):
        ...
        try:
            http.post(f"{API_URL}/sendMessage", json=payload, timeout=15)
        except Exception as e:
            print(f"خطا در ارسال پیام: {safe_err(e)}")
...
TWITTER_LINK_RE = re.compile(r"(?:x\.com|twitter\.com)/\w+/status/(\d+)")


# ---- قیمت لحظه‌ای بازار ایران (دلار/طلا/نقره) — منبع‌ها از پروژه‌ی lahzenama ----

IRAN_MARKET_TIMEOUT = 10


def _fetch_talasea_style(url, referer):
    """قیمت طلا/نقره از APIهای talasea/noghresea — خروجی (قیمت به تومان، درصد تغییر ۲۴ساعته).
    هدرها همونیه که کلاینت رسمی سایت‌ها می‌فرسته؛ بدونشون بعضی وقت‌ها رد می‌شه."""
    headers = {
        "Referer": referer,
    ...
    resp = http.get(url, headers=headers, timeout=IRAN_MARKET_TIMEOUT)
    resp.raise_for_status()
    j = resp.json()
    # API قیمت رو بر حسب هزار تومان برمی‌گردونه؛ ×۱۰۰۰ می‌کنیم تا تومان بشه (مثل کد lahzenama)
    price = round(float(str(j["price"]).replace(",", "")) * 1000)
    change = round(float(j.get("change24h", 0) or 0), 2)
    return price, change


def _fetch_tabdeal_usd():
    """قیمت دلار از Tabdeal — خروجی (قیمت به تومان، درصد تغییر)."""
    resp = http.get(
        "https://api-web.tabdeal.org/r/festival/get-asset-prices/?asset_type=currency",
        timeout=IRAN_MARKET_TIMEOUT,
    )
    resp.raise_for_status()
    for item in resp.json():
        if item.get("price_title") == "دلار":
            price = round(float(str(item["last_price"]).replace(",", "")))
            change = round(float(item.get("price_change", 0) or 0), 2)
            return price, change
    raise RuntimeError("دلار تو پاسخ Tabdeal پیدا نشد")


def handle_iran_market(chat_id):
    """قیمت لحظه‌ای دلار، طلای ۱۸ عیار و نقره — هر سه منبع موازی پرسیده می‌شن."""
    jobs = {
        "💵 دلار": _fetch_tabdeal_usd,
        "🪙 طلای ۱۸ عیار (گرم)": lambda: _fetch_talasea_style(
            "https://api.talasea.ir/api/market/getGoldPrice", "https://talasea.ir/"),
        "⚪ نقره (گرم)": lambda: _fetch_talasea_style(
            "https://api.noghresea.ir/api/market/getSilverPrice", "https://noghresea.ir/"),
    }
    results = {}
    with ThreadPoolExecutor(max_workers=3) as pool:
        ...
        for name, fut in futures.items():
            try:
                ...
                log.warning(f"iran market: {name} شکست خورد: {e}")
                results[name] = None

    if not any(results.values()):
        send_message(chat_id, "الان نتونستم قیمت بازار رو بگیرم؛ چند دقیقه دیگه دوباره امتحان کن.")
        return

    lines = ["💰 قیمت لحظه‌ای بازار ایران:", ""]
    for name, res in results.items():
        if res is None:
            lines.append(f"{name}: فعلاً در دسترس نیست")
            continue
        price, change = res
        arrow = "🔺" if change > 0 else ("🔻" if change < 0 else "➖")
        lines.append(f"{name}: {price:,} تومان ({arrow} {abs(change)}٪ در ۲۴ ساعت)")
    send_message(chat_id, "\n".join(lines))


def handle_xvideo_link(chat_id, link):
    match = TWITTER_LINK_RE.search(link) or re.search(r"status/(\d+)", link)
    if not match:
        send_message(chat_id, "لینک درست نیست. باید شبیه این باشه:\nhttps://x.com/user/status/1234567890")
        return
    tweet_id = match.group(1)
    try:
        resp = http.get(f"https://api.fxtwitter.com/status/{tweet_id}", timeout=20)
        resp.raise_for_status()
        tweet_data = resp.json().get("tweet", {})
        videos = (tweet_data.get("media", {}) or {}).get("videos") or []
        if not videos:
            send_message(chat_id, "ویدیویی تو این پست پیدا نشد.")
            return
        best = max(videos, key=lambda v: v.get("bitrate", 0))
        video_url = best["url"]
    except Exception as e:
        send_message(chat_id, f"خطا در پیدا کردن ویدیو: {safe_err(e)}")
        return
    try:
        http.post(f"{API_URL}/sendVideo",
                       json={"chat_id": chat_id, "video": video_url}, timeout=60)
    except Exception as e:
        send_message(chat_id, f"خطا در ارسال ویدیو: {safe_err(e)}")
...
def edit_message(chat_id, message_id, text, reply_markup=None):
    ...
    if reply_markup is not None:
        payload["reply_markup"] = reply_markup
    try:
        http.post(f"{API_URL}/editMessageText", json=payload, timeout=15)
    except Exception as e:
        print(f"خطا در ادیت پیام: {safe_err(e)}")
...
def answer_callback(callback_id, text=None):
    ...
    if text:
        payload["text"] = text
    try:
        http.post(f"{API_URL}/answerCallbackQuery", json=payload, timeout=15)
    except Exception as e:
        print(f"خطا در answerCallbackQuery: {safe_err(e)}")
...
def generate_image_ai_horde(prompt, api_key="0000000000", model="stable_diffusion"):
    ...
    resp = http.post(f"{AI_HORDE_BASE}/generate/async", json=payload, headers=headers, timeout=30)
    resp.raise_for_status()
    request_id = resp.json()["id"]

    for _ in range(60):  # حداکثر ۵ دقیقه صبر (۶۰ × ۵ ثانیه)
        time.sleep(5)
        check = http.get(f"{AI_HORDE_BASE}/generate/check/{request_id}", timeout=15).json()
        if check.get("done"):
            break
    ...
    status = http.get(f"{AI_HORDE_BASE}/generate/status/{request_id}", timeout=15).json()
...
def handle_update(update):
    ...
    if user_text.startswith("/summarize"):
        ...
        if target:
            try:
                ...
                send_message(chat_id, f"خطا در خوندن کانال «{target}»: {safe_err(e)}")
                return
        else:
            data = load_all()
    ...
    if user_text.startswith("/removecheck "):
        ...
        data = load_all()
        # فقط صاحب چک می‌تونه حذفش کنه (قبلاً هر کسی می‌تونست چک بقیه رو حذف کنه)
        checks = [c for c in data.get("_checks", [])
                  if not (c["id"] == check_id and c["chat_id"] == chat_id)]
        data["_checks"] = checks
        save_all(data)
        send_message(chat_id, f"چک #{check_id} حذف شد (اگه مال خودت بود).")
        return
    ...
    if TWITTER_LINK_RE.search(user_text):
        handle_xvideo_link(chat_id, user_text)
        return

    if user_text.startswith("/xvideo "):
        link = user_text[len("/xvideo "):].strip()
        handle_xvideo_link(chat_id, link)
        return

    if user_text.startswith("/translatefile"):
        link = user_text[len("/translatefile"):].strip()
        handle_url_media(chat_id, link)
        return

    if user_text.startswith("/price"):
        arg = user_text[len("/price"):].strip().lower()
        coin_map = {
            "بیت‌کوین": "bitcoin", "بیتکوین": "bitcoin", "btc": "bitcoin", "bitcoin": "bitcoin",
            "اتریوم": "ethereum", "eth": "ethereum", "ethereum": "ethereum",
            "تتر": "tether", "usdt": "tether", "tether": "tether",
            "دوج": "dogecoin", "doge": "dogecoin", "dogecoin": "dogecoin",
            "ترون": "tron", "trx": "tron", "tron": "tron",
        }
        coins = ["bitcoin", "ethereum", "tether"] if not arg else [coin_map.get(arg, arg)]
        try:
            ids = ",".join(coins)
            resp = http.get(
            ...
            send_message(chat_id, f"خطا در گرفتن قیمت: {safe_err(e)}")
            return
        if not prices:
            send_message(chat_id, "این ارز پیدا نشد. مثال: /price bitcoin")
            return
        lines = [f"{name}: ${info['usd']:,}" for name, info in prices.items()]
        send_message(chat_id, "\n".join(lines))
        return

    if user_text.startswith("/gold") or user_text.strip() in ("قیمت طلا", "💰 طلا و دلار"):
        handle_iran_market(chat_id)
        return
    ...
    if user_text.startswith("/image "):
        ...
        try:
            ...
            send_message(chat_id, f"خطا در تولید عکس: {safe_err(e)}")
            return
        send_photo(chat_id, image_url, caption=prompt[:200])
        return

    if user_text.startswith("/code "):
        prompt = user_text[len("/code "):].strip()
        data = load_all()
        extra_providers = data.get("_providers", [])
        code_prompt = (
            "تو یک برنامه‌نویس ارشدی. با دقت، بدون حدس زدن، و با بررسی edge caseها جواب بده. "
            "کد رو کامل و قابل‌اجرا بده، نه خلاصه یا ناقص:\n\n" + prompt
        )
        try:
            reply, provider, dead_providers = get_ai_response(
                [{"role": "user", "content": code_prompt}],
                extra_providers=extra_providers, topic="code", max_tokens=3000,
            )
        except AllProvidersFailed:
            send_message(chat_id, "الان هیچ سرویس هوش مصنوعی رایگانی در دسترس نیست، یکم دیگه امتحان کن.")
            return
        if dead_providers:
            data["_providers"] = [p for p in data.get("_providers", []) if p["name"] not in dead_providers]
            save_all(data)
        if len(reply) > 3500:
            send_document(chat_id, "code_output.txt", reply)
    ...
    if user_text == "/help":
        send_message(chat_id,
            ...
            "/code پرامپت - سوال کدنویسی، با providerهای رایگان و پرامپت تخصصی برنامه‌نویسی\n"
            "/price - قیمت لحظه‌ای بیت‌کوین/اتریوم/تتر (یا /price bitcoin برای یه ارز خاص)\n"
            "💰 /gold - قیمت لحظه‌ای دلار، طلای ۱۸ عیار و نقره‌ی بازار ایران (تومان)\n"
            "/xvideo لینک - دانلود ویدیو از پست X/توییتر\n"
            "📹 /translatefile لینک - ترجمه‌ی فایل صوتی/ویدیویی بزرگ (تا ۳۰۰ مگابایت) از لینک مستقیم\n\n"
    ...
    if user_text.startswith("/moa "):
        ...
        send_message(chat_id, f"در حال گرفتن نظر از {len(all_providers)} سرویس (موازی)...")
        ...
        def _ask_one(p):
            try:
                ...
                log.warning(f"moa: {p['name']} شکست خورد: {safe_err(e)}")
                return None

        # همه‌ی سرویس‌ها همزمان پرسیده می‌شن — زمان از N×۱۰ ثانیه به ~۱۰ ثانیه می‌رسه
        opinions = []
        with ThreadPoolExecutor(max_workers=min(6, len(all_providers))) as pool:
            for res in pool.map(_ask_one, all_providers):
                if res:
                    opinions.append(res)
    ...
    try:
        ...
        for url in data.get("_learned_urls", []):
            try:
                ...
                print(f"خطا در خوندن لینک یادگرفته‌شده {url}: {safe_err(e)}")
        ...
        print(f"همه‌ی providerها شکست خوردن: {safe_err(e)}")
        if e.dead_providers:
...
def send_photo(chat_id, file_id, caption=""):
    try:
        http.post(f"{API_URL}/sendPhoto",
                      json={"chat_id": chat_id, "photo": file_id, "caption": caption},
                      timeout=15)
    except Exception as e:
        print(f"خطا در ارسال عکس: {safe_err(e)}")
...
def check_reminders():
    ...
    if not checks:
        return
    # تاریخ به وقت تهران، نه UTC — اختلاف ۳:۳۰ می‌تونس�� یادآوری رو یه روز جابه‌جا کنه
...
# قفل per-chat: پیام‌های هر چت پشت‌سرهم پردازش می‌شن (ترتیب و سازگاری حفظ می‌شه)،
# ولی چت‌های مختلف موازی — پردازش ویدیوی یه نفر، بقیه رو معطل نمی‌کنه.
...
def _process_update_safe(update):
    with _chat_locks[_chat_id_of(update)]:
        try:
            ...
            log.exception(f"خطا در پردازش پیام: {safe_err(e)}")
...
def main():
    ...
    try:
        http.get(f"{API_URL}/deleteWebhook", timeout=15)
    except Exception as e:
        print(f"خطا در deleteWebhook: {safe_err(e)}")

    start_time = time.time()
    offset = None
    last_reminder_check = 0
    executor = ThreadPoolExecutor(max_workers=HEAVY_WORKERS)
    print("بات روشن شد...")
    ...
    while time.time() - start_time < MAX_RUNTIME_SECONDS:
        if time.time() - last_reminder_check > 3600:
            try:
                ...
                print(f"خطا در چک یادآوری چک‌ها: {safe_err(e)}")
        ...
        try:
            resp = http.get(f"{API_URL}/getUpdates", params=params,
                                 timeout=POLL_TIMEOUT + 10)
            resp.raise_for_status()
            updates = resp.json().get("result", [])
        except Exception as e:
            print(f"خطا در getUpdates: {safe_err(e)}")
            time.sleep(5)
            continue

        for update in updates:
            offset = update["update_id"] + 1
            executor.submit(_process_update_safe, update)

        # هر دور polling، تغییرات ذخیره‌نشده تو Gist نوشته می‌شه (debounce طبیعی)
        try:
            storage.flush()
        except Exception as e:
            print(f"خطا در flush ذخیره‌سازی: {safe_err(e)}")

    # صبر می‌کنیم کارهای در حال اجرا تموم شن و آخرین تغییرات ذخیره شه
    executor.shutdown(wait=True)
    try:
        storage.flush()
    except Exception as e:
        print(f"خطا در flush پایانی: {safe_err(e)}")
