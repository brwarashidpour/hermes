...
def parse_vless_full(link):
    """کل جزئیات یه لینک vless:// رو درمیاره — برای ساخت فایل JSON کامل."""
...
def build_xray_json(parsed):
    """یه فایل JSON استاندارد Xray/V2Ray می‌سازه که NapsternetV نسخه‌های جدید
    مستقیم می‌تونه ایمپورت کنه (بدون نیاز به فرمت اختصاصی npv4)."""
    outbound = {
        "tag": "proxy",
        "protocol": "vless",
        "settings": {
...
def handle_new_proxy_configs(chat_id, found_links):
    """کانفیگ‌های vless:// جدید رو ذخیره می‌کنه و برای هرکدوم فایل JSON آماده‌ی
    ایمپورت تو NapsternetV می‌سازه و می‌فرسته."""
    data = load_all()
    ...
    my_proxies[chat_id] = existing + new_unique
    data["_my_proxies"] = my_proxies
    save_all(data)

    for i, link in enumerate(new_unique, 1):
        parsed = parse_vless_full(link)
        if not parsed:
            continue
        config_json = build_xray_json(parsed)
        filename = f"config_{i}.json" if len(new_unique) > 1 else "config.json"
        send_document(chat_id, filename, json.dumps(config_json, indent=2, ensure_ascii=False))

    send_message(chat_id,
        f"{len(new_unique)} کانفیگ جدید ذخیره و به فایل JSON تبدیل شد (مجموع ذخیره‌شده: "
        f"{len(my_proxies[chat_id])}).\nفایل رو تو NapsternetV از «Import» → «From file» انتخاب کن. "
        f"برای تست سرعت همه‌ی کانفیگ‌های ذخیره‌شده: /myproxies")
...
def handle_update(update):
    ...
    if user_text == "/help":
        send_message(chat_id,
            ...
            "🔗 کانفیگ‌های vless:// خودتو (تک یا چندتایی) مستقیم بفرست تا ذخیره بشن\n"
