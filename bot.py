...
def handle_update(update):
    ...
    if user_text.startswith("/addproxy"):
        ...
        new_unique = [link for link in found if link not in existing]
        my_proxies[chat_id] = existing + new_unique
        data["_my_proxies"] = my_proxies
        save_all(data)
        send_message(chat_id, f"{len(new_unique)} کانفیگ جدید ذخیره شد (مجموع: {len(my_proxies[chat_id])}).")
        return
    ...
    if VLESS_LINE_RE.search(user_text) and not user_text.startswith("/"):
        ...
        save_all(data)
        send_message(chat_id,
            f"{len(new_unique)} کانفیگ جدید ذخیره شد (مجموع: {len(my_proxies[chat_id])}). "
            f"برای گرفتن سریع‌ترین‌ها: /myproxies")
        return
