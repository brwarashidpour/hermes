...
GROK_BUILD_TIMEOUT = 250  # ثانیه، زیر سقف Telegram/حلقه‌ی اصلی


def run_grok_build_cli(prompt):
    """grok-build رو headless (بدون رابط گرافیکی) با یه پرامپت صدا می‌زنه.
    رو یه پوشه‌ی موقت اجرا می‌شه که تصادفی فایل‌های خود ریپوی بات رو دستکاری نکنه."""
    if not XAI_API_KEY:
        raise RuntimeError("XAI_API_KEY تنظیم نشده")

    env = {**os.environ, "XAI_API_KEY": XAI_API_KEY}
    with tempfile.TemporaryDirectory() as tmpdir:
        result = subprocess.run(
            ["grok", "-p", prompt],
            capture_output=True,
            text=True,
            timeout=GROK_BUILD_TIMEOUT,
            env=env,
            cwd=tmpdir,
        )

    if result.returncode != 0:
        raise RuntimeError((result.stderr or "خطای نامشخص از grok-build")[:500])

    return result.stdout.strip() or "(خروجی خالی برگشت)"
...
def handle_update(update):
    ...
    if user_text.startswith("/code "):
        ...
        send_message(chat_id, "در حال اجرای grok-build (نصب کامل CLI)... ممکنه چند دقیقه طول بکشه.")
        try:
            output = run_grok_build_cli(prompt)
        except subprocess.TimeoutExpired:
            send_message(chat_id, "⏰ زمان اجرا تموم شد، پرامپت رو کوتاه‌تر یا ساده‌تر امتحان کن.")
            return
        except FileNotFoundError:
            send_message(chat_id, "grok-build رو این ماشین نصب نیست — چک کن workflow.yml مرحله‌ی نصبش رو داره.")
            return
        except Exception as e:
            send_message(chat_id, f"❌ خطا: {e}")
            return
        send_document(chat_id, "grok_output.txt", output)
        if len(output) < TELEGRAM_MAX_LEN:
    ...
    if user_text == "/help":
        send_message(chat_id,
            ...
            "/code پرامپت - اجرای grok-build (CLI کامل نصب‌شده، برای کارهای سنگین کدنویسی)\n\n"
